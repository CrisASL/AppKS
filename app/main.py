...
def cargar_cubo_excel(archivo, tipo_cubo: str, key_prefix: str = ""):
    ...
    # ========================================================================
    # 🔍 DEBUGGING - ANÁLISIS DE COLUMNAS
    # ========================================================================
    st.markdown("---")
    st.subheader("🔍 DEBUG - Análisis de Columnas")
    
    # Mostrar todas las columnas detectadas
    with st.expander("📋 Todas las columnas detectadas", expanded=True):
        st.write(f"**Total de columnas:** {len(df.columns)}")
        st.code(df.columns.tolist())
    
    # Buscar columnas que contengan palabras clave según tipo de cubo
    palabras_clave = {
        'requisiciones': ['talca', 'concepcion', 'rancagua', 'copiapo', 'viña'],
        'compras': ['numoc', 'proveedor', 'fecha'],
        'ventas': ['codprod', 'ene', 'feb', 'mar'],
        'inventario': ['talca', 'stock', 'bodega']
    }
    
    if tipo_cubo in palabras_clave:
        st.write(f"**Buscando columnas con palabras clave de '{tipo_cubo}':**")
        for palabra in palabras_clave[tipo_cubo]:
            cols_encontradas = [col for col in df.columns if palabra.lower() in str(col).lower()]
            if cols_encontradas:
                st.success(f"✅ '{palabra}': {cols_encontradas}")
            else:
                st.warning(f"⚠️ '{palabra}': No encontrada")
    ...
    with st.expander("👁️ Muestra de datos (primeras 3 filas)"):
    ...
    if validador:
        ...
        if es_valido and not columnas_faltantes:
            ...
            with st.expander("📋 Columnas opcionales faltantes", expanded=False):
                st.warning(f"Las siguientes {len(columnas_faltantes)} columnas no están presentes:")
                for col in columnas_faltantes[:10]:  # Mostrar máximo 10
                    st.write(f"• {col}")
                if len(columnas_faltantes) > 10:
                    st.write(f"... y {len(columnas_faltantes) - 10} más")
                st.info("ℹ️ El cubo es funcional, pero algunas características pueden estar limitadas.")
        else:
            # Error crítico
            st.error(mensaje)
...
def seccion_carga_cubos():
    ...
    with col1:
        st.subheader("📋 Cubo de Requisiciones")
            ...
            if df is not None:
                ...
                with st.expander("🔍 Diagnóstico de Columnas de Fecha", expanded=False):
                    ...
                    if 'FechaOC' in df.columns:
                        ...
                        st.write(f"- Valores no nulos: {df['FechaOC'].notna().sum()}/{len(df)}")
                        ...
                        if pd.api.types.is_numeric_dtype(df['FechaOC']):
                            st.warning("⚠️ FechaOC está como número (serial de Excel). Se convertirá al cargar.")
                            # Mostrar ejemplo de conversión
                            ejemplo_serial = df['FechaOC'].dropna().iloc[0] if len(df['FechaOC'].dropna()) > 0 else None
                            if ejemplo_serial:
                                try:
                                    fecha_convertida = pd.to_datetime(ejemplo_serial, unit='D', origin='1899-12-30')
                                    st.info(f"Ejemplo: {ejemplo_serial} → {fecha_convertida.strftime('%Y-%m-%d')}")
                                except:
                    ...
                    if 'FechaRecepcion' in df.columns:
                        ...
                        st.write(f"- Valores no nulos: {df['FechaRecepcion'].notna().sum()}/{len(df)}")
                        if pd.api.types.is_numeric_dtype(df['FechaRecepcion']):
                            st.warning("⚠️ FechaRecepcion está como número (serial de Excel). Se convertirá al cargar.")
                
                # Botón para insertar en base de datos (idempotente)
                if st.button("💾 Cargar a Base de Datos", key="btn_cargar_compras", type="primary"):
                        ...
                        try:
                            ...
                            with st.spinner("Insertando datos en base de datos..."):
                                ...
                                with compras_service.get_db_connection() as conn:
                                    insertados, actualizados, sin_cambios, errores = compras_service.cargar_compras_desde_dataframe(
                                ...
                                if errores:
                                    with st.expander(f"⚠️ Errores ({len(errores)})", expanded=False):
                                ...
                                st.info("🔄 Actualizando requisiciones con datos de compras...")
                                with st.spinner("Sincronizando requisiciones con seguimiento OC..."):
                                    exito_sync, mensaje_sync, actualizados_sync = db.actualizar_requisiciones_desde_compras()
                                    
                                    if exito_sync:
                                        st.success(f"✅ {actualizados_sync} requisiciones actualizadas con datos de compras")
                                    else:
                ...
                st.info("💡 **Carga Idempotente con UPSERT**: Puedes cargar el mismo cubo múltiples veces. Los registros nuevos se insertarán, los existentes se actualizarán si hay cambios, o se mantendrán sin cambios si son idénticos.")
        
        st.markdown("---")
        
        st.subheader("📦 Cubo de Inventario")
...
def pagina_dashboard():
    ...
    with col4:
        valor_formateado = utils.formatear_numero(stats['valor_total_oc'], simbolo_moneda="$")
        st.metric(
            "Valor Total OC",
            valor_formateado,
...
def pagina_gestion_requisiciones():
    ...
    # Mensaje informativo
    st.info("💡 Las requisiciones se cargan automáticamente al importar el cubo de requisiciones en el Dashboard")
    ...
    with st.expander("🔧 Herramientas", expanded=False):
        ...
        with col_btn:
            if st.button("🔄 Actualizar desde Compras", type="primary", use_container_width=True):
                with st.spinner("Actualizando requisiciones desde compras..."):
                    exito, mensaje, actualizados = db.actualizar_requisiciones_desde_compras()
...
# ============================================================================
# FUNCIÓN DESHABILITADA - Las requisiciones se cargan automáticamente desde el cubo
# ============================================================================
# def formulario_nueva_requisicion():
#     """Formulario para crear una nueva requisición."""
#     st.subheader("➕ Crear Nueva Requisición")
#     
#     with st.form("form_nueva_req"):
#         col1, col2, col3 = st.columns(3)
#         
#         with col1:
#             numreq = st.text_input("N° Requisición *", max_chars=config.MAX_LEN_NUMREQ)
#             codprod = st.text_input("Código Producto *", max_chars=config.MAX_LEN_CODPROD)
#             cantidad = st.number_input("Cantidad *", min_value=1, value=1, step=1)
#         
#         with col2:
#             desprod = st.text_input("Descripción Producto")
#             proveedor = st.text_input("Proveedor")
#             oc = st.text_input("N° OC", max_chars=config.MAX_LEN_NUMOC)
#         
#         with col3:
#             fecha_oc = st.date_input("Fecha OC", value=None)
#             n_guia = st.text_input("N° Guía de Despacho")
#             estado_oc = st.selectbox("Estado", config.ESTADOS_OC)
#         
#         observacion = st.text_area(
#             "Observaciones",
#             max_chars=config.MAX_LEN_OBSERVACION,
#             help="Máximo 500 caracteres"
#         )
#         
#         detalle = st.text_area(
#             "Detalle",
#             max_chars=config.MAX_LEN_DETALLE,
#             help="Máximo 500 caracteres"
#         )
#         
#         submitted = st.form_submit_button("💾 Guardar Requisición", width='stretch')
#         
#         if submitted:
#             # Validaciones
#             if not numreq or not codprod:
#                 st.error(config.MSG_WARN_CAMPOS_REQUERIDOS)
#                 return
#             
#             es_valido_cant, msg_cant = utils.validar_cantidad(cantidad)
#             if not es_valido_cant:
#                 st.error(msg_cant)
#                 return
#             
#             if fecha_oc:
#                 es_valida_fecha, msg_fecha = utils.validar_fecha_no_futura(str(fecha_oc))
...
#             try:
#                 req_id = db.crear_requisicion(datos_requisicion)
#                 if req_id > 0:
#                     st.success(config.MSG_EXITO_CREAR)
#                     st.balloons()
#                 else:
#                     st.error(config.MSG_ERROR_CREAR)
#             except Exception as e:
#                 st.error(f"{config.MSG_ERROR_CREAR}: {str(e)}")
# ============================================================================
...
def tabla_listado_requisiciones():
    ...
    with st.expander("🔍 Filtros", expanded=False):
        st.caption("💡 Los filtros de fecha aplican sobre la **Fecha de Requisición** (FEmision del cubo)")
        ...
        if st.button("🔄 Limpiar Filtros", type="secondary", key="btn_limpiar_filtros_req"):
            st.session_state.filtro_req_estado = []
            st.session_state.filtro_req_fecha_desde = None
            st.session_state.filtro_req_fecha_hasta = None
            st.session_state.filtro_req_solo_pendientes = False
    ...
    if 'df_req_original' not in st.session_state or st.session_state.get('reload_req_data', False):
...
def pagina_seguimiento_oc():
    ...
    try:
        ...
        with compras_service.get_db_connection() as conn:
            ...
            if not tabla_existe:
                st.warning("⚠️ La tabla de compras no existe. Carga primero el Cubo de Compras.")
                return
            ...
            with col_filtro1:
                ...
                if st.session_state.filtro_oc_seleccionada in lista_ocs:
                    index_oc = lista_ocs.index(st.session_state.filtro_oc_seleccionada) + 1
                
                oc_seleccionada = st.selectbox(
                    "Filtrar por OC:",
                    options=["Todas"] + lista_ocs,
                    index=index_oc if st.session_state.filtro_oc_seleccionada != "Todas" else 0,
            ...
            with col_filtro2:
                # Filtro por estado
                estados_disponibles = list(stats['por_estado'].keys()) if stats['por_estado'] else []
                
                # Encontrar el índice del valor guardado
                index_estado = 0
                if st.session_state.filtro_estado_seleccionado in estados_disponibles:
                    index_estado = estados_disponibles.index(st.session_state.filtro_estado_seleccionado) + 1
                
                estado_seleccionado = st.selectbox(
                    "Filtrar por Estado:",
                    options=["Todos"] + estados_disponibles,
                    index=index_estado if st.session_state.filtro_estado_seleccionado != "Todos" else 0,
...
def pagina_configuracion():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["💾 Backup", "📊 Información del Sistema", "📋 Historial de Cargas", "🗑️ Limpiar Datos"])
    ...
    with tab1:
        ...
        try:
            if os.path.exists(config.BACKUP_PATH):
                backups = [f for f in os.listdir(config.BACKUP_PATH) if f.endswith('.db')]
                ...
                if backups:
                    ...
                    for backup in backups[:10]:  # Mostrar últimos 10
                        ...
                        fecha_mod = datetime.fromtimestamp(os.path.getmtime(ruta_backup))
                        
                        st.text(f"📄 {backup} - {tamanio:.1f} KB - {fecha_mod.strftime('%d/%m/%Y %H:%M')}")
                else:
    ...
    with tab3:
        ...
        if not df_historial.empty:
            ...
            df_display = df_historial[['fecha_carga', 'registros_leidos', 'registros_insertados', 
                                       'registros_omitidos', 'errores', 'usuario']].copy()
            
            df_display.columns = ['Fecha', 'Leídos', 'Insertados', 'Omitidos', 'Errores', 'Usuario']
    ...
    with tab4:
        ...
        **Úsala solo si quieres empezar completamente desde cero.**
        ...
        if not confirmacion:
            st.warning("⚠️ Marca la casilla de confirmación para habilitar el botón de limpieza total")
