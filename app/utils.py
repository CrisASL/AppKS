...
def validar_cubo_requisiciones(df: pd.DataFrame) -> Tuple[bool, str, List[str]]:
    ...
    return validar_estructura_cubo(
        df, config.COLUMNAS_REQ, config.COLUMNAS_CRITICAS_REQ, "Cubo de Requisiciones"
    )
...
def validar_cubo_compras(df: pd.DataFrame) -> Tuple[bool, str, List[str]]:
    ...
    return validar_estructura_cubo(
        df, config.COLUMNAS_COMPRAS, config.COLUMNAS_CRITICAS_COMPRAS, "Cubo de Compras"
    )
...
def validar_cubo_ventas(df: pd.DataFrame) -> Tuple[bool, str, List[str]]:
    ...
    return validar_estructura_cubo(
        df, config.COLUMNAS_VENTAS, config.COLUMNAS_CRITICAS_VENTAS, "Cubo de Ventas"
    )
...
def cargar_excel_con_selector_hoja(archivo, tipo_cubo: str, key_prefix: str = ""):
    ...
    try:
        ...
        st.info(
            f"📋 El archivo tiene {len(hojas_disponibles)} hojas. Selecciona la hoja correcta:"
        )
...
def determinar_estado_por_saldo(
    cantidad: int, cantidad_recibida: int, estado_actual: str = None
) -> str:
    """
...
def analizar_stock_disponible(
    ...
    if bodega_sucursal in producto:
        resultado["stock_sucursal"] = (
            int(producto[bodega_sucursal]) if pd.notna(producto[bodega_sucursal]) else 0
        )
    ...
    if "Total general" in producto:
        resultado["stock_total"] = (
            int(producto["Total general"]) if pd.notna(producto["Total general"]) else 0
        )
    ...
    if resultado["stock_sucursal"] >= cantidad_requerida:
        resultado["sugerencia"] = (
            f"✅ Stock suficiente en {bodega_sucursal} ({resultado['stock_sucursal']} unidades)"
        )
        ...
        if bodega_max[1] >= cantidad_requerida:
            resultado["sugerencia"] = (
                f"🔄 Transferir desde {bodega_max[0]} (stock: {bodega_max[1]} unidades)"
            )
            resultado["bodega_sugerida"] = bodega_max[0]
        else:
            resultado["sugerencia"] = (
                f"⚠️ Stock insuficiente. Disponible: {resultado['stock_total']} | Requerido: {cantidad_requerida}"
            )
    else:
        resultado["sugerencia"] = (
            f"🛒 Generar OC - Stock total insuficiente ({resultado['stock_total']} unidades disponibles)"
        )

    return resultado


def sugerir_accion(codprod: str, cantidad_req: int, inventario_df: pd.DataFrame) -> str:
    """
...
def obtener_productos_criticos(
    inventario_df: pd.DataFrame, umbral: int = config.STOCK_CRITICO
) -> pd.DataFrame:
    """
    ...
    criticos = inventario_df[
        (inventario_df["KS TALCA"] > 0) & (inventario_df["KS TALCA"] < umbral)
    ].copy()
...
def formatear_fecha(
    fecha_str: str, formato_entrada: str = "%Y-%m-%d", formato_salida: str = None
) -> str:
    """
...
def generar_nombre_exportacion(
    prefijo: str = "requisiciones", extension: str = "xlsx"
) -> str:
    """
...
def filtrar_cubo_requisiciones_sucursal(
    df: pd.DataFrame, sucursal: str = "Talca"
) -> pd.DataFrame:
    """
...
def extraer_requisiciones_pendientes(
    cubo_req_df: pd.DataFrame, cubo_compras_df: pd.DataFrame, sucursal: str = "Talca"
) -> pd.DataFrame:
    """
    ...
    req_pendientes = req_sucursal[
        ["NumReq", "CodProd", "DesProd", sucursal, "FEmision"]
    ].copy()
...
def generar_alertas_oc(requisiciones_df: pd.DataFrame) -> List[Dict]:
    ...
    for _, req in requisiciones_df.iterrows():
        ...
        if req.get("estado_oc") == "Recepción Parcial" and req.get(
            "fecha_modificacion"
        ):
...
def validar_codigo_producto(codprod: str) -> Tuple[bool, str]:
    ...
    if len(codprod) > config.MAX_LEN_CODPROD:
        return (
            False,
            f"El código de producto no puede exceder {config.MAX_LEN_CODPROD} caracteres",
        )
...
def formatear_numero(
    numero: float, decimales: int = 0, simbolo_moneda: str = None
) -> str:
    """
    ...
    try:
        ...
        numero_formateado = (
            numero_formateado.replace(",", "X").replace(".", ",").replace("X", ".")
        )
...
def obtener_config_columnas_editables() -> Dict[str, any]:
    ...
    for campo in config.CAMPOS_NO_EDITABLES_UI:
        column_config[campo] = st.column_config.Column(
            disabled=True, help=f"Campo protegido: no editable desde la interfaz"
        )
    ...
    column_config["estado_envio"] = st.column_config.SelectboxColumn(
        "Estado Envío",
        help="Estado de envío de la OC al proveedor (editable)",
        options=config.ESTADOS_ENVIO,
        required=False,
    )
...
def preparar_df_para_edicion_segura(df: pd.DataFrame) -> pd.DataFrame:
    ...
    columnas_restantes = [
        col for col in df_copia.columns if col not in columnas_disponibles
    ]
    ...
    # Normalizar estado_envio a string con valor por defecto
    if "estado_envio" in df_copia.columns:
        df_copia["estado_envio"] = (
            df_copia["estado_envio"].fillna("No Enviado").astype(str)
        )

    return df_copia


def validar_ediciones_antes_de_guardar(
    ...
    if "fecha_oc" in df_editado.columns:
        try:
            fechas_futuras = df_editado[
                pd.to_datetime(df_editado["fecha_oc"], errors="coerce")
                > pd.Timestamp.now()
            ]
